...
- **GameScene 연결**: `initializeSystems()`에서 World 생성, `game-config.json`의 `entityTypes`/`abilities` 배열 기반 동적 entity type/ability 등록 후 SystemPlugin 파이프라인 구성 → `assertConfigSyncOrThrow()`로 `entityPipeline` 동기화 검증(fail-fast) → `startAll()`에서 `InitialEntitySpawnSystem.start()`가 `game-config.json`의 `initialEntities` 순서대로 플레이어 등 초기 엔티티를 data-driven 스폰. `cleanup()`에서 `world.clear()` 호출. 커서 위치는 `world.transform.get('player')` 에서 읽음.
