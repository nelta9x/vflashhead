...
- **`src/scenes/BootScene.ts`**: 초기 로딩 화면. 에셋 프리로딩(오디오, SVG 아이콘), 프로그레스 바 표시. 아이콘 preload 대상은 `abilities.json.active[].icon`을 기준으로 data-driven 계산되며, 아이콘 파일 누락 시 경고 후 런타임을 계속 진행한다.
...
- **`src/scenes/GameScene.ts`**: **핵심 게임 루프 오케스트레이터**. 시스템/렌더러를 초기화하고 `update()`에서 파이프라인을 실행합니다.
  - **initializeSystems() 순서**: ServiceRegistry → ServicePlugins → `abilities.json.active` + `game-config.json.entityTypes` 기반 동적 등록 → `AbilityConfigSyncValidator` fail-fast 검증 → SystemPlugin 파이프라인 구성(includes `core:initial_spawn`) → `assertConfigSyncOrThrow()`(파이프라인 구성 드리프트 fail-fast) → `startAll()` (player 등 초기 엔티티 data-driven 스폰) → GameSceneController 생성.
...
- **`ModSystemRegistry.ts`** (`src/plugins/`): MOD 커스텀 시스템 등록/실행 레지스트리. `registerSystem(id, tickFn, priority?)` + `bindSystemEventBus(systemId, bus)`를 제공하며, `runAll(delta, sharedContext)` 실행 시 시스템별로 바인딩된 `ScopedEventBus`를 컨텍스트에 주입한다. 미바인딩 시스템은 즉시 예외를 던진다.
...
- **플러그인 colocate 시스템들** (`src/plugins/builtin/systems/`): 플러그인 구체 렌더러/엔티티를 직접 참조하거나 콘텐츠 서비스에 의존하는 시스템. SystemPlugin과 같은 디렉토리에 colocate.
  ...
  - `SpaceshipProjectileSystem` (`core:spaceship_projectile`): 우주선 엔티티의 발사체 스폰/이동/커서 충돌/렌더링. 우주선별 발사 타이머(Map<EntityId, FireState>), 플레이어 방향 + aimVariance 각도 오차 발사, 무적 쿨다운 충돌 판정.
...
- **`EntitySystemPipeline.ts`** (`src/systems/`): data-driven 엔티티 시스템 실행 파이프라인. `game-config.json`의 `entityPipeline` 배열이 실행 순서의 SSOT (21개 시스템). `register(system)`, `unregister(id)`, `setEnabled(id, enabled)`, `run(delta)`. config 순서대로 배치 → config에 없는 등록 시스템은 끝에 추가. `getMissingSystems()`, `getUnmappedSystems()`, `getRegisteredIds()` 진단 메서드와 `assertConfigSyncOrThrow()` fail-fast 검증 메서드를 제공.
  - GameScene 호출 순서: `syncWorldContext()` → `entitySystemPipeline.run(delta)` (21개 시스템 순차, 모든 tick 로직 포함)
  - 파이프라인 순서: initial_spawn → wave → combo → status_effect_tick → entity_status → entity_timing → player → ability_tick → entity_movement → boss_reaction → boss_coordinator → magnet → cursor_attack → spaceship_projectile → black_hole → orb → falling_bomb → health_pack → entity_visual → entity_render → mod_tick
...
- **GameScene 연결**: `initializeSystems()`에서 World 생성, `abilities.json.active`/`game-config.json.entityTypes` 기반 동적 ability/entity type 등록 후 `AbilityConfigSyncValidator`로 설정 동기화 검증(fail-fast) → SystemPlugin 파이프라인 구성 → `assertConfigSyncOrThrow()`로 `entityPipeline` 동기화 검증(fail-fast) → `startAll()`에서 `InitialEntitySpawnSystem.start()`가 `game-config.json`의 `initialEntities` 순서대로 플레이어 등 초기 엔티티를 data-driven 스폰. `cleanup()`에서 `world.clear()` 호출. 커서 위치는 `world.transform.get('player')` 에서 읽음.
...
- **`ModRegistry.ts`**: MOD 라이프사이클 관리자. **스냅샷 diff**로 `registerMod()` 전후 레지스트리 상태를 비교하여 MOD가 등록한 ability/entityType/modSystem/entitySystem/archetype/store를 추적한다. `loadMod()`는 diff로 확인된 `modSystemIds`에 MOD의 `ScopedEventBus`를 시스템별 바인딩하고, 바인딩 실패 시 전체 롤백한다. `unloadMod()` / `unloadAll()` 시 diff 기반 일괄 해제 + ScopedEventBus 구독 정리.
...
- **`builtin/abilities/`**: 내장 어빌리티 플러그인 (CursorSize, CriticalChance, Missile, HealthPack, Magnet, ElectricShock, Orb, BlackHole) 및 저주 업그레이드 (GlassCannon, Berserker, Volatility). `ABILITY_FACTORIES` factory map + `registerBuiltinAbilities(definitions)` 패턴으로 `abilities.json.active` 기반 동적 등록(`pluginId` lookup + `definition.id` strict 검증). 저주 업그레이드는 `isCurse: true` 플래그로 구분되며 긍정/부정 효과를 동시에 제공합니다.
- **`builtin/entities/`**: 내장 엔티티 타입 플러그인 (PlayerEntity, BasicDish, BombEntity, SpaceshipPlugin, GatekeeperSpaceship). `ENTITY_TYPE_FACTORIES` factory map + `registerBuiltinEntityTypes(ids)` 패턴으로 `game-config.json`의 `entityTypes` 배열 기반 동적 등록. PlayerEntity는 `singleton` 카테고리로 풀링 없이 단일 인스턴스만 존재. SpaceshipPlugin은 dish 아키타입을 사용하며 `onUpdate`에서 SpaceshipRenderer로 비주얼을 오버라이드. **엔티티 전용 렌더러도 colocate**: `BossRenderer`, `BossShatterEffect`, `LaserRenderer`, `MenuBossRenderer`, `DishRenderer`, `SpaceshipRenderer`, `CursorRenderer`, `CursorTrail`.
...
- **데이터 파일 목록 (`data/*.json`)**:
  - `game-config.json`: 전역 설정, 기본 언어(`defaultLanguage`), 플레이어 스탯, UI 레이아웃, 폰트 설정, 레이저 공격, 자기장 설정, **렌더 레이어 깊이(`depths`)** — 모든 `setDepth()` 값의 SSOT. **`entityPipeline`**: 21개 엔티티 시스템 실행 순서 배열 (초기 스폰 1개 + 게임 레벨 6개 + 엔티티 14개). **`systemPlugins`**: 서비스/시스템 플러그인 활성화 목록. **`entityTypes`**: 활성화할 빌트인 엔티티 타입 ID 배열 (factory map 기반 동적 등록). **`initialEntities`**: 게임 시작 시 data-driven 스폰할 엔티티 타입 ID 배열.
  - `abilities.json`: 활성 어빌리티 SSOT. `active[]`에서 `id`(런타임 ID) + `pluginId`(factory lookup) + `upgradeId`(업그레이드 매핑) + `icon`(Boot preload 메타)을 함께 정의한다.
