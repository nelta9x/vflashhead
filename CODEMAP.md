...
- **`src/scenes/BootScene.ts`**: 초기 로딩 화면. 에셋 프리로딩(오디오, 프로시저럴 그래픽), 프로그레스 바 표시.
- **`src/scenes/MenuScene.ts`**: 메인 메뉴. 타이틀, 시작 버튼, 별 배경, 보스 애니메이션, 그리드 효과, 커서 트레일, 메뉴용 접시 스폰 및 입력 처리.
...
- **`Dish.ts`**: 주요 적인 '접시'.
  ...
  - 업그레이드 옵션 지원 (데미지 보너스, 공격속도, 크리티컬, 슬로우, 커서 크기).
- **`Boss.ts`**: 보스 몬스터. 코어 비주얼(Arc)과 아머 조각 렌더링, HP 비율에 따른 시각적 변화, `MONSTER_HP_CHANGED`/`MONSTER_DIED` 이벤트 구독.
...
- **`src/ui/`**:
  ...
  - `AbilityPanel`: 현재 보유한 업그레이드 목록 표시 (DOM 기반, `UPGRADES_CHANGED` 이벤트 구독).
...
- **`src/data/types.ts`**: 모든 JSON 데이터 구조에 대한 TypeScript 인터페이스 정의 (`ScreenConfig`, `PlayerConfig`, `ComboConfig`, `WavesConfig`, `DishesConfig`, `UpgradesConfig`, `BossConfig`, `MenuConfig`, `FeedbackConfig` 등).
- **`src/data/constants.ts`**: JSON 기반 데이터 중 코드에서 자주 쓰이는 물리/기하학적 상수 (`GAME_WIDTH`, `GAME_HEIGHT`, `COLORS`, `COLORS_HEX`, `FONTS`, `CURSOR_HITBOX`, `SPAWN_AREA` 등).
- **`src/data/game.config.ts`**: Phaser 엔진 기술 설정 (물리, 렌더링, 스케일, 오디오 등).
- **데이터 파일 목록 (`data/*.json`)**:
  - `game-config.json`: 전역 설정, 플레이어 스탯, UI 레이아웃, 폰트 설정, 레이저 공격, 자기장 설정.
  ...
  - `health-pack.json`: 힐팩 스폰 확률, 낙하 속도, 히트박스, 회복량 설정.
...
| **플레이어 상태** | `HP_CHANGED` | 데미지/회복 발생 시 | `HealthSystem`, `UpgradeSystem` | `HealthPackSystem`, `GameScene` |
