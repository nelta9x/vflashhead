...
- **`MonsterSystem.ts`**: 보스 몬스터의 HP 관리 및 사망 로직.
...
- **주요 데이터 파일**:
  - `game-config.json`: 전역 설정, 플레이어 스탯.
  - `dishes.json`: 적 종류별 체력, 포인트, 속도.
  - `waves.json`: 웨이브별 구성 및 난이도 곡선.
  - `upgrades.json`: 업그레이드 어빌리티 정의 및 확률.
  - `feedback.json`: 연출용 수치 (흔들림 강도, 파티클 개수 등).
...
| 이벤트 명 | 발생 시점 | 주요 구독자 |
|-----------|-----------|-------------|
| `DISH_DESTROYED` | 접시 파괴 시 | `WaveSystem`, `ComboSystem`, `GameScene`(피드백) |
| `WAVE_COMPLETED` | 모든 접시 처리 시 | `GameScene`(UI 표시), `WaveSystem` |
| `HP_CHANGED` | 데미지/회복 발생 시 | `HUD`, `GameScene`(피드백) |
| `GAUGE_UPDATED` | 게이지 수치 변경 시 | `GameScene`(커서 연출) |
| `PLAYER_ATTACK` | 게이지 완충 후 공격 시 | `GameScene`(보스 공격 연출) |
