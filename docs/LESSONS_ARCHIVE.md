...
> 이 파일은 `LESSONS.md`의 원본 상세 항목을 보존한 아카이브입니다.
> 요약본은 [LESSONS.md](LESSONS.md)를 참고하세요.

프로젝트 개발 중 발견한 버그와 교훈을 기록합니다.

---

## 2026-02-10: 신규 위협 동시 등장 + 레이저 간격 급변은 별도 축으로 분리해야 난이도 곡선이 안정된다

### 증상
- W3("첫 번개")에서 보스 레이저와 낙하폭탄이 동시에 등장해 초반 밸런스 스파이크가 발생.
- W5→W6, W8→W9 구간에서 레이저 간격이 한 번에 -2500~-3000ms 급락해 체감 난이도가 불연속적이었음.

### 원인
- 낙하폭탄 `minWave`가 레이저 첫 등장 웨이브(W3)와 같아 두 위협이 한꺼번에 학습 부담을 줌.
- 레이저 간격 변화량이 웨이브별로 균일하지 않아 "쉬어가기 → 급가속" 패턴이 반복됨.

### 해결
- 낙하폭탄 `minWave`를 3→5로 이동해 레이저 학습 구간(W3~W4)을 확보.
- W5~W14 레이저 간격을 완만화해 최대 스텝을 -1600/-2500(최종웨이브만)으로 제한.

### 교훈
- **신규 위협 축(레이저/낙하폭탄/신규 접시)은 등장 시점을 최소 1~2웨이브 간격으로 분리해야 학습 곡선이 안정된다.**
- **레이저 간격 같은 연속 수치는 웨이브 간 변화량(Δ)을 일정 범위 이내로 유지해야 체감 급변을 막을 수 있다.**
...
## 2026-02-09: 폭탄 처치 가속 버프는 `짧은 지속 + 스택 상한 + 만료 갱신`을 함께 고정해야 의도된 템포만 남는다

### 증상
- 금구슬에 폭탄 처치 보상을 붙일 때, 만료/중첩 규칙이 분명하지 않으면 체감이 너무 약하거나 반대로 상시 가속으로 굳어질 위험이 있었음.

### 원인
- `speedMultiplier`만 적용하면 버프 창이 거의 느껴지지 않거나, 폭탄 밀집 구간에서 의도보다 오래 유지되어 기본 밸런스를 잠식하기 쉬움.
- 폭탄 제거 소스가 늘어날 때 `byAbility` 경로를 명시하지 않으면 기존 폭탄 피해/콤보 규칙과 충돌할 수 있음.

### 해결
- `orbiting_orb` 데이터에 `overclockDurationMs`, `overclockSpeedMultiplier`, `overclockMaxStacks`를 추가해 오버클럭 규칙을 SSOT로 고정.
- 오버클럭은 구슬이 폭탄을 제거한 순간에만 발동하고, `forceDestroy(true)` 경로로 폭탄 제거 의미를 유지.
- 스택은 처치마다 +1(상한 적용), 지속시간은 처치마다 갱신, 만료 시 즉시 0스택으로 리셋.

### 교훈
- **처치형 템포 버프는 "스택 증가/지속 갱신/상한/만료"를 세트로 정의해야 상시 버프화를 막을 수 있다.**
- **폭탄 제거 신규 소스는 계속 `byAbility=true` 의미를 고정해야 기존 피해/콤보 규칙과 충돌하지 않는다.**
...
## 2026-02-09: 블랙홀 성장 누적은 "개별 인스턴스"로 한정해야 예측 가능성이 유지됨

### 증상
- 블랙홀이 폭탄/접시를 제거할 때 성장시키는 기능을 추가하면, 누적 단위를 명시하지 않을 경우 홀 간 공유 누적이나 웨이브 간 누적이 섞여 밸런스 예측이 어려워질 수 있었음.
- 성장 반경/피해를 코드 상수로 두면 기획 조정 시 코드 수정과 회귀 위험이 반복될 수 있었음.

### 원인
- 주기 교체형 블랙홀 구조에서 "어디까지 상태를 들고 갈지"를 분리하지 않으면, 생성 타이밍/교체 타이밍에 따라 누적 의미가 흔들림.
- 성장 규칙(비율/고정 증가, 피해 증가)이 SSOT 데이터에 없으면 설명/데이터/실행 로직이 다시 어긋날 수 있음.

### 해결
- 블랙홀 레벨 데이터에 `consumeRadiusGrowthRatio`, `consumeRadiusGrowthFlat`, `consumeDamageGrowth`를 추가해 성장 규칙을 SSOT로 이동.
- 폭탄 중심 흡수(`forceDestroy(true)`)와 블랙홀 틱 피해로 일반 접시 처치 시점에만, 해당 블랙홀 인스턴스의 반경/틱 피해를 즉시 증가.
- 성장 반경은 `currentRadius * (1 + consumeRadiusGrowthRatio) + consumeRadiusGrowthFlat` 공식을 적용하고, 다음 스폰 교체 시 기본 수치로 초기화.

### 교훈
- **교체형 범위 어빌리티의 성장 누적은 인스턴스 경계(개별 객체 수명)로 고정해야 체감과 디버깅이 안정된다.**
- **성장 공식/증가량도 SSOT 데이터로 강제해야 설명 문구와 실제 전투 규칙이 함께 유지된다.**
- **폭탄 제거는 계속 `byAbility=true` 경로를 유지해야 기존 피해/콤보 규칙이 깨지지 않는다.**
...
### 증상
- 구조화 카드 도입 후 `핵심 변화 배지`가 상단 공간을 차지하면서, 6~7행 어빌리티(`금구슬`, `블랙홀`)에서 하단 수치가 진행바와 겹치거나 잘리는 문제가 발생함.
